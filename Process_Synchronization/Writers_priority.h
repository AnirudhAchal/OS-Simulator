...
//to enqueue_writer reader who starts reading
queue_writer* enqueue_writer(int ct, int index, queue_writer *head)
{
    queue_writer *temp, *temp1;
    temp = (queue_writer*)malloc(sizeof(queue_writer));
    temp->ct = ct;
    temp->index = index;
    if(head == NULL)
    {
...
//to dequeue_writer reader who completes reading
queue_writer* dequeue_writer(queue_writer *head)
{
    queue_writer *temp;
    temp = head;
...
//main function
int writers_priority(void)
{
    cout << "\nRunning Process Synchronization Writers Priority\n\n";
    int n, m, i, j, mark, curr_time, t;
    int S=0, R; 
    queue_writer* head = NULL;
    //Document *b, *r, *w;
    ...
    for(i=0 ; i<n+m;)
    {
        ...
        else
        {
            ...
            if(mark == i)
            {
                ...
                head = enqueue_writer(b[i].ct, b[i].index, head);
                b[i].q = 1;
            }
        ...
        {    
            ...
            if(b[j].wr == 0 && R==1)
            {
                ...
                R = 1;
                head = enqueue_writer(b[j].ct, b[j].index, head);
                b[j].q = 1;
                continue;
            }
            else if(b[j].w == 1)
